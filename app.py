...
def main():
    ...
    st.markdown(
        ...
        .header-title {font-size:28px; font-weight:700; color:#0b3b5a}
        .subtle {color:#556;}
    ...
    # Create tabs for a cleaner UX
    tab_data, tab_preproc, tab_eda, tab_cluster, tab_results = st.tabs(["Data", "Preprocessing", "EDA", "Clustering", "Results"])
    ...
    with tab_preproc:
        ...
        st.write(f"Jumlah baris sebelum pembersihan: {jumlah_sebelum}")
        st.write(f"Jumlah baris setelah pembersihan: {len(data)}")
    ...
    # Plots: histograms and boxplots using plotly
        ...
        for kolom in fitur_clustering:
            ...
            fig2 = px.box(data_agregasi, x=kolom, points='all', title=f'Boxplot {kolom}')
    ...
    for xcol, ycol in pasangan_variabel:
        ...
        fig_corr = px.imshow(matriks_korelasi, text_auto='.2f', title='Matriks Korelasi Variabel Clustering')
