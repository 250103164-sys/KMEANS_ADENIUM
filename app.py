...
def main():
    ...
    st.sidebar.markdown(
        ...
        **Panduan singkat**
        ...
        - Lihat hasil di halaman Results
        """
    )
    ...
    if data.empty:
        st.warning('Tidak ada data valid setelah preprocessing. Periksa dataset Anda.')
        return
    ...
    if menu_utama == "Preprocessing":
    ...
    elif menu_utama in ["EDA", "Clustering", "Results"]:
        data['VARIETAS'] = (
        ...
        if data_agregasi.empty:
            st.warning('Tidak ada agregasi data yang tersedia.')
            return
        ...
        if menu_utama == "Results":
            opsi_cluster = sorted(hasil_clustering['CLUSTER'].unique())
            if opsi_cluster:
                cluster_selected = st.sidebar.selectbox('Pilih cluster', options=opsi_cluster)
            else:
                st.sidebar.warning('Tidak ada cluster untuk dipilih.')
        ...
        if menu_utama == "Clustering":
            ...
            if nilai_silhouette is not None:
        ...
        elif menu_utama == "Results":
            st.markdown('<div class="card">', unsafe_allow_html=True)
            st.write('### Detail cluster')
            if cluster_selected is None:
                st.warning('Pilih cluster terlebih dahulu di sidebar.')
            else:
